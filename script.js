document.addEventListener('DOMContentLoaded', () => {
  ...
  // פונקציה לביצוע חישוב והצגת התוצאות
  function doCompute() {
  ...
  // הפעלת חישוב בלחיצה על אנטר בשדות תאריך
  ...
  // אירוע לכפתור חזור - מעבר למסך הקלט ועצירת מעקב
  document.getElementById('backBtn').addEventListener('click', () => {
    document.getElementById('inputScreen').classList.remove('hidden');
    document.getElementById('resultScreen').classList.add('hidden');

    if (window.millisecondsInterval) {
      clearTimeout(window.millisecondsInterval);
    }
  });

  // עדכון מאיות השנייה ופרטי הזמן בזמן אמת
